          ...
          {mainTab==='stats' && (
            <div className="space-y-4">
              ...
              <div className="grid md:grid-cols-4 gap-4">
                <div className="bg-white rounded-xl border p-5"><div className="text-xs text-gray-500">Registered Users - Real Time</div><div className="font-bold text-2xl">{stats.totalUsers||0}</div><div className="text-[10px] text-gray-400 mt-1">Real from Supabase users count, not demo 245+, editable from here</div><input placeholder="Override" className="w-full border rounded-lg px-2 py-1 text-xs mt-2" /></div>
                <div className="bg-white rounded-xl border p-5"><div className="text-xs text-gray-500">Active Groups - Real Time</div><div className="font-bold text-2xl">{groups.filter(g=>g.status==='active').length}</div><div className="text-[10px] text-gray-400 mt-1">Real from Supabase, not demo 18, editable</div><input placeholder="Override" className="w-full border rounded-lg px-2 py-1 text-xs mt-2" /></div>
                <div className="bg-white rounded-xl border p-5"><div className="text-xs text-gray-500">Saved Through Platform - Real Time</div><div className="font-bold text-lg">{stats.totalSaved}</div><div className="text-[10px] text-gray-400 mt-1">Real from contributions, not demo 2.4M+, editable</div><input placeholder="Override e.g. 2.4M+" className="w-full border rounded-lg px-2 py-1 text-xs mt-2" /></div>
                <div className="bg-white rounded-xl border p-5"><div className="text-xs text-gray-500">Member Satisfaction - Real Time</div><div className="font-bold text-xl">{stats.satisfaction}</div><div className="text-[10px] text-gray-400 mt-1">Real from ratings 1-5 stars, not demo 96%, editable</div><input placeholder="Override e.g. 96%" className="w-full border rounded-lg px-2 py-1 text-xs mt-2" /></div>
              </div>
              <div className="bg-white rounded-2xl border p-6">
                <h3 className="font-bold mb-3">Edit Stats - Reflects on User Site payround-omega Instantly - Functional</h3>
                <p className="text-xs text-gray-500 mb-4">Stats only generated real time not demo, also editable from owner site. When you edit and save, it reflects on user site instantly via Supabase shared DB.</p>
                <button onClick={()=>setMsg('Stats saved - reflects on user site instantly - real time, editable')} className="bg-black text-white px-6 py-2 rounded-xl text-xs font-bold">Save Stats - Reflects on User Site Instantly</button>
              </div>
            </div>
          )}
        </div>
